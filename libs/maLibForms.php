...
function mkTable($tabData,$listeChamps=false,$titres=null)
{

	//le paramètre titres permet de changer les nom des colonnes du tableau 
	...
	echo "<table border=\"1\" id=\"tab_annuaire\" class=\"table table-responsive table-striped table-bordered table-hover table-condensed\">\n";
	// afficher une ligne d'entete avec le nom des champs

	if( ! $titres )
		mkLigneEntete($tabData[0],$listeChamps);
	else
		mkLigneEntete($tabData[0],$titres);
...
function mkSelect($nomChampSelect, $tabData,$champValue, $champLabel,$info=null, $selected=false,$champLabel2=false)
{

	$multiple=""; 
	if (preg_match('/.*\[\]$/',$nomChampSelect)) $multiple =" multiple =\"multiple\" ";

	echo "<select $multiple $info name=\"$nomChampSelect\">\n";
	foreach ($tabData as $data)
	{
